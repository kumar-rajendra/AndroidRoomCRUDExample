...
public class AddUserActivity extends AppCompatActivity {
    ...
    static class AddUser extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            //ud.userDao().insert(u);
            Long userid = ud.userDao().insert(u);
            Library lib = new Library();
            lib.setUserId(userid);
            lib.setBook_name(u.getFirstName()+" Algoritham"+userid);
            lib.setBook_type("Data_Structure "+ userid);
            ud.userDao().insertLibrary(lib);
            //UserAndLibrary uObj = ud.userDao().getUserAndLibrary(userid);
            //Log.i("LibTag",uObj.user.toString() + " | " + uObj.LibList.get(0).book_name);
